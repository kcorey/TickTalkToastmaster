class TimerApp {
    constructor() {
        ...
        // Also detect Chrome on iOS (which also uses WebKit)
        ...
        // Add special handling for iOS Chrome
        this.handleIOSChromeAudio();        
    }
    ...
    playBellSound() {
        ...
        if (this.isIOS) {
            ...
            if (this.iosSounds && this.iosSounds.bell) {
                console.log('Using iOS-specific bell sound');
                this.iosSounds.bell.currentTime = 0;
                this.iosSounds.bell.volume = 1.0;
                this.iosSounds.bell.play()
                    .then(() => console.log('iOS bell sound played successfully'))
                    .catch(e => {
                        console.error('iOS bell sound error:', e);
                        alert('Sound playback failed. Please click "Enable Sound" button first.');
                    });
            } else {
                console.error('iOS bell sound not initialized properly');
            }
        } else {
            // Try to use bell.mp3 for all devices for consistency
            if (this.bellSound) {
                console.log('Using standard bell sound element');
                this.bellSound.currentTime = 0;
                this.bellSound.volume = 1.0;
                this.bellSound.play()
                    .then(() => console.log('Bell sound played successfully'))
                    .catch(e => console.error('Error playing bell sound:', e));
            } else {
                // Try to create a new audio element if bellSound is not available
                console.log('Creating fallback bell sound');
                const bellSound = new Audio('sounds/bell.mp3');
                bellSound.volume = 1.0;
                bellSound.play()
                    .then(() => console.log('Fallback bell sound played successfully'))
                    .catch(e => console.error('Error playing fallback bell sound:', e));
            }
        }
    }
    ...
    setupIOSAudioFix() {
        ...
        audioUnlockBtn.innerText = "Enable Sound";
        ...
        // Create iOS-specific sound objects
        this.iosSounds = {
            bell: new Audio('sounds/bell.mp3')
        };
        
        // Apply iOS-specific attributes
        this.iosSounds.bell.preload = 'auto';
        this.iosSounds.bell.controls = false;
        
        // Add event listener for the unlock button
        audioUnlockBtn.addEventListener('click', () => {
            // Try to unlock the bell sound
            const bellSound = this.iosSounds.bell;
            bellSound.volume = 0.1; // Low volume for testing
            
            bellSound.play()
                .then(() => {
                    bellSound.pause();
                    bellSound.currentTime = 0;
                    bellSound.volume = 1.0; // Reset to full volume
                    console.log('iOS bell sound unlocked');
                    
                    // Test sound with a short delay
                    setTimeout(() => {
                        this.playTestSound();
                    }, 500);
                ...
                .catch(e => {
                    console.error('Failed to unlock iOS sound:', e);
                    alert('Failed to unlock audio. Please try again or check device settings.');
                });
        });
    }

    // Add a test sound method
    playTestSound() {
        if (this.isIOS && this.iosSounds && this.iosSounds.bell) {
            this.iosSounds.bell.currentTime = 0;
            this.iosSounds.bell.volume = 0.5;
            this.iosSounds.bell.play()
    ...
    // Add special handling for iOS Chrome
    handleIOSChromeAudio() {
        if (!this.isIOSChrome) return;
        
        console.log('Setting up special handling for iOS Chrome');
        
        // Create a more prominent audio unlock button for Chrome
        const chromeUnlockBtn = document.createElement('button');
        chromeUnlockBtn.innerText = "Enable Sound in Chrome";
        chromeUnlockBtn.style.position = "fixed";
        chromeUnlockBtn.style.top = "50%";
        chromeUnlockBtn.style.left = "50%";
        chromeUnlockBtn.style.transform = "translate(-50%, -50%)";
        chromeUnlockBtn.style.padding = "15px 25px";
        chromeUnlockBtn.style.background = "#4285F4"; // Chrome blue
        chromeUnlockBtn.style.color = "white";
        chromeUnlockBtn.style.border = "none";
        chromeUnlockBtn.style.borderRadius = "4px";
        chromeUnlockBtn.style.zIndex = "10000";
        chromeUnlockBtn.style.boxShadow = "0 2px 5px rgba(0,0,0,0.3)";
        chromeUnlockBtn.style.fontSize = "16px";
        
        // Add event listener
        chromeUnlockBtn.addEventListener('click', () => {
            // Create and play audio elements for Chrome
            const unlockAudio = new Audio('sounds/bell.mp3');
            unlockAudio.volume = 0.1;
            
            unlockAudio.play()
                .then(() => {
                    console.log('iOS Chrome audio unlocked');
                    chromeUnlockBtn.style.display = 'none';
                    
                    // Show success message
                    const msg = document.createElement('div');
                    msg.innerText = "Sound enabled in Chrome!";
                    msg.style.position = "fixed";
                    msg.style.top = "10px";
                    msg.style.left = "50%";
                ...
                .catch(e => {
                    console.error('Failed to unlock iOS Chrome audio:', e);
                    alert('Please try again or check your device settings.');
                });
        });
        
        // Add to DOM
        document.body.appendChild(chromeUnlockBtn);
    }

    // Add this method to your TimerApp class
    setupAboutDialog() {
        const helpIcon = document.getElementById('help-icon');
        const aboutDialog = document.getElementById('about-dialog');
        const closeButton = document.getElementById('close-dialog');
        
        if (!helpIcon || !aboutDialog || !closeButton) {
            console.error('About dialog elements not found');
            return;
        }
        
        // Show dialog when help icon is clicked
...
class TimerApp {
    constructor() {
        ...
        document.addEventListener('keydown', (event) => {
            if (event.key === 'Escape' && aboutDialog.classList.contains('visible')) {
    ...
    setupAboutDialog() {
        ...
        // Also hide dialog when clicking outside of it
